...
startup: the public **`realvirtual-Library`** GitHub repository
([`github.com/game4automation/realvirtual-Library`](https://github.com/game4automation/realvirtual-Library)).
It appears as a catalog tab named *realvirtual-Library* alongside any
user-added catalogs. The branch is pinned, so startup costs exactly one
GitHub tree API call. Additional catalogs can be appended with the
`?library=<url>` URL parameter (repeatable) or added at runtime through the
Catalog tab.
